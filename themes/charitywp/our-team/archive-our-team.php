...
?>
	...
	<section class="content-area">
		...
		<?php
			...
			$post_per_row = $theme_options_data['thim_our_team_post_per_row'];
			$col = 12/$post_per_row;
			$number = $post_per_row = $theme_options_data['thim_our_team_post_per_page'];
			$paged  = get_query_var( 'paged' ) ? intval( get_query_var( 'paged' ) ) : 1;
			$offset	= ($paged == 1) ? 1 : (($paged-1) * $number);
			$team_args = array(
				'post_type'     	=> 'our_team',
				...
				<div class="thim-our-team">
					...
					$i = 0;
						while ( $members->have_posts() ): 
							...
							if ('show' === $theme_options_data['thim_our_team_display_link']) {
								$show_social = true;
							}
							if ($regency != '') {
							...
							if ( has_post_thumbnail() ) {
								...
								if ($show_social){
									...
									if ('show' === $theme_options_data['thim_our_team_display_facebook']) {
										$html .= $face_url;
									}
									if ('show' === $theme_options_data['thim_our_team_display_twitter']) {
										$html .= $twitter_url;
									}
									if ('show' === $theme_options_data['thim_our_team_display_rss']) {
										$html .= $rss_url;
									}
									if ('show' === $theme_options_data['thim_our_team_display_skype']) {
										$html .= $skype_url;
									}
									if ('show' === $theme_options_data['thim_our_team_display_dribbble']) {
										$html .= $dribbble_url;
									}
									if ('show' === $theme_options_data['thim_our_team_display_linkedin']) {
										$html .= $linkedin_url;
									}
							...
							$html .= $regency;
							if ('show' === $theme_options_data['thim_our_team_display_phone']) {
								$html .= $our_team_phone;
							}
							if ('show' === $theme_options_data['thim_our_team_display_email']) {
								$html .= $our_team_email;
							}
							$html .= '		</div>';

							if ('show' === $theme_options_data['thim_our_team_display_content']) {
