	...
	<?php if ( is_single() ): ?>
		...
		if ( class_exists( 'TP_Event_Authentication' ) ) {
			...
			if ( absint( $event->quantity ) != 0 && $event->post->post_status !== 'tp-event-expired' ) { ?>
				<!--<a href="#" class="thim-button style3 thim-toggle-div" data-div=".event_register_area" data-scroll="true"><?php esc_html_e('RESERVA AHORA', 'charitywp'); ?></a>-->
				<a href="#event-form-popup" data-mfp-src="#event-form-popup" class="thim-button style3 thim-toggle-div open-popup-link"><?php esc_html_e('RESERVA AHORA', 'charitywp'); ?></a>
			<?php }
		}
		echo "</div>";
		?>
