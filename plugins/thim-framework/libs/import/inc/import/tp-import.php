...
if ( intval( $max_time_limit ) < 120 ) {
	...
	if ( set_time_limit( 0 ) ) {
		...
		$response = array(
			...
			'next'    => 'error',
			'return'  => false,
			'message' => esc_html__( 'Sorry to let you know that your server system can not work properly with Demo Importer due to some configs. Please request some support.', 'thim-framework' ),
		);
		echo json_encode( $response );
		exit();
	}
