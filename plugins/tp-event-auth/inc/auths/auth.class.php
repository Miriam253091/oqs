...
class Auth extends Module_Base {
	...
	function event_auth_register( $atts = array(), $content = null  ) {
		...
		remove_filter( 'register_url', array( $this, 'register_url' ), 99, 1 );
		$atts = wp_parse_args( $atts, array(
	...
	function forgot_pass( $atts = array(), $content = null  ) {
		...
		// remove filter
		remove_filter( 'lostpassword_url', array( $this, 'lostpassword_url' ), 99, 3 );
		...
		$html = ob_get_clean();
		// add filter
		add_filter( 'lostpassword_url', array( $this, 'lostpassword_url' ), 99, 3 );
